...
const char* host = "finch.am";
String path = "/";

unsigned char letsencrypt_ca[] = {
...
void setup() {
  ...
  Serial.print("Setting the clock...");
  configTime(43200, 0, "0.pool.ntp.org", "1.pool.ntp.org", "2.pool.ntp.org"); // 43200 = +12 NZ time zone, 0 = NZST
  while (!time(nullptr)) {
     delay(500);    
     Serial.print(".");
  }  
  ...
  client.setCACert(letsencrypt_ca, sizeof(letsencrypt_ca));
  if (!client.verifyCertChain(host)) {
    Serial.println("Couldn't verify the certificate :(");
    return;
  }

  Serial.println("Making something that looks like an HTTP GET...");
